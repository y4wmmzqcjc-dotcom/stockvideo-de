...
// ========== AUTHENTICATION MODULE ==========
        const auth = {
            ...
            sendOTP(settings) {
                ...
                if (typeof emailjs === 'undefined') {
                    // EmailJS not loaded ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ show OTP in console for dev/testing
            ...
            verifyOTP() {
                ...
                if (parseInt(inputOTP) === this.currentOTP) {
                    ...
                    if (this.otpAttempts >= this.maxOTPAttempts) {
                        ...
                        alert.innerHTML = '<div class="alert alert-error">UngÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¼ltiger Code. Bitte versuchen Sie es erneut.</div>';
                    }
                }
            },
        ...
        const admin = {
            ...
            refreshDeployStatus() {
                ...
                .then(data => {
                    if (data.commit) {
                        document.getElementById('statDeployStatus').textContent = 'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ';
            ...
            renderVideosList() {
                ...
                if (this.videos.length === 0) {
                    container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¬</div><div class="empty-state-title">Keine Videos</div><p>Erstellen Sie Ihr erstes Video</p></div>';
                    return;
                }
                ...
                container.innerHTML = this.videos.map((video, idx) => `
                    <div class="list-item">
                        ...
                        <div class="list-item-content">
                            ...
                            <div class="list-item-meta">${video.category} ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ ${video.resolution} ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¬${video.prices.web}</div>
                        </div>
                        <div class="list-item-actions">
                            <button class="button button-small button-secondary" onclick="admin.editVideo(${idx})">Bearbeiten</button>
                            <button class="button button-small button-danger" onclick="admin.deleteVideo(${idx})">LÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶schen</button>
                        </div>
                    </div>
                `).join('');
            },
            ...
            updateCategoryDropdown() {
                ...
                dropdown.innerHTML = '<option value="">-- WÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¤hlen Sie eine Kategorie --</option>' +
            ...
            deleteVideo(idx) {
                if (confirm('Wirklich lÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶schen?')) {
                    this.videos.splice(idx, 1);
                    localStorage.setItem('adminVideos', JSON.stringify(this.videos));
                    localStorage.setItem('adminLastChange', new Date().toISOString());
                    this.loadVideos();
                    this.showAlert('videosAlert', 'success', 'Video gelÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶scht');
                }
            },
            ...
            renderCategoriesList() {
                ...
                if (this.categories.length === 0) {
                    container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ</div><div class="empty-state-title">Keine Kategorien</div><p>Erstellen Sie Ihre erste Kategorie</p></div>';
                    return;
                }
                ...
                container.innerHTML = this.categories.map((cat, idx) => `
                    <div class="list-item">
                        ...
                        <div class="list-item-actions">
                            ...
                            <button class="button button-small button-danger" onclick="admin.deleteCategory(${idx})">LÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶schen</button>
                        </div>
                    </div>
                `).join('');
            },
            ...
            deleteCategory(idx) {
                if (confirm('Wirklich lÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶schen?')) {
                    ...
                    localStorage.setItem('adminLastChange', new Date().toISOString());
                    this.loadCategories();
                    this.showAlert('categoriesAlert', 'success', 'Kategorie gelÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶scht');
                }
            },
            ...
            loadPageContent() {
                ...
                this.content = stored ? JSON.parse(stored) : {
                    ...
                    features: [
                        { icon: 'ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¬', title: '', description: '' },
                        { icon: 'ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ', title: '', description: '' },
                        { icon: 'ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¡', title: '', description: '' },
                        { icon: 'ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ°', title: '', description: '' }
                    ],
                    pricing: [
                ...
                pricingContainer.innerHTML = (this.content.pricing || []).map((p, idx) => `
                    <div class="form-section" style="margin-bottom: 1rem;">
                        ...
                        <div class="form-row">
                            ...
                            <div>
                                <label class="form-label">AuflÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶sung</label>
                        ...
                        <div class="form-row">
                            <div>
                                ...
                                <input type="text" class="form-input" value="${p.price}" placeholder="ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¬ 99.99" data-pricing-price="${idx}">
                            </div>
            ...
            resetDesignToDefault() {
                if (confirm('Wirklich zurÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¼cksetzen?')) {
            ...
            changePassword() {
                ...
                if (newPassword !== confirmPassword) {
                    this.showAlert('settingsAlert', 'error', 'PasswÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶rter stimmen nicht ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¼berein');
                    return;
                }
                ...
                this.showAlert('settingsAlert', 'success', 'Passwort geÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¤ndert');
            },
            ...
            publishToGitHub() {
                ...
                button.textContent = 'VerÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶ffentlichung...';
                ...
                this.createGitHubCommit(settings, config)
                    .then(() => {
                        this.showAlert('dashboardAlert', 'success', 'Erfolgreich verÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶ffentlicht! Cloudflare Pages wird neu gebaut...');
                        setTimeout(() => {
                            this.refreshDeployStatus();
                        }, 3000);
                    })
                    .catch(err => {
                        ...
                        this.showAlert('dashboardAlert', 'error', 'Fehler beim VerÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶ffentlichen: ' + err.message);
                    })
                    .finally(() => {
                        button.disabled = false;
                        button.textContent = 'VerÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¶ffentlichen';
                    });
            },
...
(function(){
  ...
  var ensure = function(){
    ...
    admin.publishToGitHub = async function() {
      try {
        var pw = prompt('Admin-Passwort zum VerÃÂ¶ffentlichen:', 'admin123');
        if (!pw) return;
...
;try{if(typeof auth!=="undefined")window.auth=auth;if(typeof admin!=="undefined")window.admin=admin;}catch(e){}

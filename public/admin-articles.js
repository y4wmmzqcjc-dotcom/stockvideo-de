...
window.adminArticles = {
  ...
  _renderBlockEditor() {
    ...
    // checks is array of {key, label, ok}
    if (Array.isArray(checks)) {
      for (var k = 0; k < checks.length; k++) {
        var chk = checks[k];
        seoHtml += '<div class="we-check ' + (chk.ok ? 'we-check-ok' : 'we-check-fail') + '">';
        seoHtml += '<div class="we-check-icon">' + (chk.ok ? '\u2713' : '\u2717') + '</div>';
        seoHtml += '<span>' + chk.label + '</span>';
        seoHtml += '</div>';
      }
    }

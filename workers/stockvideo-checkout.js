  ...
  async fetch(request, env) {
    ...
    if (path === '/admin/r2-put' && request.method === 'PUT') {
      ...
      return jsonResponse({ ok: true, key: key, size: buf.byteLength, contentType: ct });
        ...
        if (path === '/admin/data' && request.method === 'POST') { const body = await request.json(); const kind = body.kind; if (kind !== 'videos' && kind !== 'categories') return jsonResponse({ error: 'bad kind' }, 400); if (!Array.isArray(body.items)) return jsonResponse({ error: 'items must be array' }, 400);
          // Server-side unmojibake: undo accidental UTF-8-over-Latin-1 loops before writing to GitHub.
          const __unwrap = (s) => { try { return new TextDecoder('utf-8', { fatal: true }).decode(Uint8Array.from(s, c => { const cc = c.charCodeAt(0); if (cc > 0xFF) return null; return cc; })); } catch (e) { return null; } };
          const __fix = (s) => { if (typeof s !== 'string' || !/[\u00C0-\u00FF]/.test(s)) return s; for (let i = 0; i < 30; i++) { const next = __unwrap(s); if (next == null || next === s) break; s = next; } return s; };
          const __walk = (n) => { if (n == null) return n; if (typeof n === 'string') return __fix(n); if (Array.isArray(n)) return n.map(__walk); if (typeof n === 'object') { const o = {}; for (const k of Object.keys(n)) o[k] = __walk(n[k]); return o; } return n; };
          body.items = __walk(body.items); const p = 'public/data/' + kind + '.json'; const existing = await ghGet(p); const content = JSON.stringify(body.items, null, 2); const res = await ghPut(p, content, 'admin: update ' + kind + '.json (' + body.items.length + ' items)', existing && existing.sha); if (res.status >= 300) return jsonResponse({ error: 'github', status: res.status, msg: res.json.message }, 502); return jsonResponse({ ok: true, commit: res.json.commit && res.json.commit.sha }); }

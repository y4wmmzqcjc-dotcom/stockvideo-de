  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path.startsWith('/admin/')) {
        ...
        const ghGet = async (p) => { const r = await fetch('https://api.github.com/repos/' + GH_REPO + '/contents/' + p + '?ref=' + GH_BRANCH, { headers: ghHeaders }); if (!r.ok) return null; const j = await r.json(); return { sha: j.sha, content: new TextDecoder('utf-8').decode(Uint8Array.from(atob(j.content.replace(/\n/g,'')), c => c.charCodeAt(0))) }; };
        ...
        if (path === '/admin/r2-rename' && request.method === 'POST') {
          ...
          for (const item of body.renames) {
            ...
            try {
              ...
              if (!obj) {
                results.push({ oldKey, newKey, ok: false, note: 'not found in R2' });
                continue;
              }
